...
namespace Locust.Shetab.Parsian
{
    public class ParsianPaymentProvider : BaseShetabPaymentProvider
    {
        ...
        {
            var result = new PaymentProviderBeginPaymentResponse<ParsianBankTranStatus>(Now);
        ...
        protected override Task<PaymentProviderEndPaymentResponse> endPaymentAsync(IDictionary<string, string> request, string code, string status, CancellationToken cancellation)
        {
            ...
            try
            {
                ...
                pec.PaymentEnquiryCompleted += new PaymentEnquiryCompletedEventHandler((sender, args) =>
                {
                    if (args.Error != null)
                    {
                        ...
                        if (!args.Cancelled)
                        {
                            try
                            {
                                var result = new PaymentProviderEndPaymentResponse<ParsianBankTranStatus>(Now);
                ...
                {
                    var result = new PaymentProviderEndPaymentResponse<ParsianBankTranStatus>();
