...
namespace Locust.WebTools
{
    ...
    public class ECMAScriptPacker : IHttpHandler, IJavascriptObfuscator
    {
        ...
        private int BitStringToInt(string bits)
        {
            var reversedBits = Reverse(bits);
            var num = 0;
        ...
        private string compressKeywords(StringCollection keywords)
        {
            var result = "";
            ...
            foreach (string word in keywords)
            {
                ...
                {
                    var bin = "00000000" + Convert.ToString(lookup64.IndexOf(ch), 2);
            ...
            result = sb.ToString();
            var bytes = new byte[result.Length / 8];
            for (var i = 0; i < result.Length; i += 8)
            {
                var bin = result.Substring(i, 8);
                var x = (byte)BitStringToInt(bin);
                bytes[i / 8] = x;
            }
        ...
        private string bootStrap(string packed, WordList keywords)
        {
            ...
            var _keywords = sbKeywords.ToString();
            //var ck = compressKeywords(keywords.Sorted);
            //Debug.WriteLine(ck);
            string keywordsout = _keywords + "'.split('|')";
        ...
        private static string lookup64 = lookup62 + "_|";
